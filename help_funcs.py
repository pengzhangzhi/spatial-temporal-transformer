...
def save_dict_to_binary_file(data, path):
...
def save_test_results(test_results, experiment_path):
    """save all test results to a binary file `test_result.pkl` and save overall test results to error_performance.csv.

    Args:
        test_results (_type_): _description_
        experiment_path (_type_): _description_
    """
    name = "test_result.pkl"
    path = os.path.join(experiment_path, name)
    save_dict_to_binary_file(test_results, path)
    mse, rmse, mae, mape, ape = get_error_performance(test_results["overall"])
    error_performance = {"mse": mse, "rmse": rmse, "mae": mae, "mape": mape, "ape": ape}
    pd.DataFrame(error_performance, index=[0]).to_csv(
        os.path.join(experiment_path, "error_performance.csv")
    )
