...
class Mydataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.normalization:
            ...
            xc, xt, y = list(
                map(
                    lambda x: rearrange(x, "l n h w ->  n l h w"),
...
def get_loaders(args, pretrain=False):
    ...
    nw = (
        0
        if running_window()
        else min([os.cpu_count(), batch_size if batch_size > 1 else 0, 8])
    )
...
def train(args, model=None, experiment_path=None):
    ...
    for epoch in range(args.epochs):
        ...
        # validate
        val_loss, val_rmse, val_ape = evaluate(
        ...
        # test
        MSE, y_rmse, y_mae, y_mape, ape = test(
            model=model, data_loader=test_loader, device=device, args=args
        )
        # append a error list so as to save a error list.
        test_results = [MSE, y_rmse * args.m_factor_2, y_mae, y_mape, ape]
...
if __name__ == "__main__":
    ...
    args.epochs = opt.epochs if opt.epochs else args.epochs
    if opt.noPretrain:
