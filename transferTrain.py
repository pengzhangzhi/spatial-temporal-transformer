...
@Project -> File：spatial-temporal-transformer-main->transferTrain
...
from utils import train_one_epoch, evaluate, test, running_platform, running_window, loadTransferModelParam
...
def train(args, model=None, experiment_path=None):
    ...
    if model is None:
        ...
        model = model
    paramPath = os.path.join(args.transferModelDir,"best_model.pt")
    loadTransferModelParam(paramPath,model)
...
if __name__ == '__main__':
    ...
    parser.add_argument("-c", '--config-name', type=str, default="TaxiBJ10")
